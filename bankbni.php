...
class BankBNI extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $config = Configuration::getMultiple(array('BANK_BNI_DETAILS', 'BANK_BNI_OWNER', 'BANK_BNI_ADDRESS'));
        ...
        $this->displayName = $this->l('Bank BNI');
        $this->description = $this->l('Accept payments for your products via Bank BNI transfer.');
    ...
    public function install()
    {
        ...
        $newState->module_name = 'bankbni';
        $newState->invoice = false;
        $newState->color = "#EF4406";
        ...
        foreach ($languages as $language) {
            if ($language['iso_code'] == 'id') {
                $newState->name[(int)$language['id_lang']] = 'Menunggu pembayaran via Bank BNI';
            } else {
                $newState->name[(int)$language['id_lang']] = 'Awaiting Bank BNI Payment';
            }
        ...
        if ($newState->add()) {
            ...
            foreach ($languages as $language) {
                if ($language['iso_code'] == 'id') {
                    copy(dirname(__FILE__).'/mails/id/bankbni.html', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankbni.html');
                    copy(dirname(__FILE__).'/mails/id/bankbni.txt', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankbni.txt');
                } else {
                    copy(dirname(__FILE__).'/mails/en/bankbni.html', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankbni.html');
                    copy(dirname(__FILE__).'/mails/en/bankbni.txt', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankbni.txt');
                }
            }
        } else {
            return false;
    ...
    {
        if (!Configuration::deleteByName('BANK_BNI_DETAILS') || !Configuration::deleteByName('BANK_BNI_OWNER') || !Configuration::deleteByName('BANK_BNI_ADDRESS') || !parent::uninstall()) {
            return false;
        }
        return true;
    }
    ...
    public function hookDisplayPaymentEU($params)
    {
        ...
        $payment_options = array(
            'cta_text' => $this->l('Pay by Bank BNI'),
    ...
    public function hookPaymentReturn($params)
    {
        ...
        if (in_array($state, array(Configuration::get('PS_OS_BANKBNI'), Configuration::get('PS_OS_OUTOFSTOCK'), Configuration::get('PS_OS_OUTOFSTOCK_UNPAID')))) {
            $this->smarty->assign(array(
    ...
    {
        $this->context->controller->addCSS(($this->_path).'bankbni.css', 'all');
    }
}
